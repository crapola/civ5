...
-- MyMod
local g_NationalInstanceManager = InstanceManager:new( "ProdButton", "Button", Controls.NationalWonderButtonStack );
...
-- MyMod
local nationalHeadingOpen = true;
...
function OnNationalWonderHeaderSelected()
	nationalHeadingOpen = not nationalHeadingOpen;
	UpdateWindow( GetCurrentCity() );
end
...
function UpdateWindow( city )
	...
	-- MyMod
	g_NationalInstanceManager:ResetInstances();
			...
			if city:CanConstruct( buildingID, 0, 1 ) then
				...
				if col == 2 then
					...
					if col==3 then
						...
						table.insert(listNational, building);
					end
				end
			end
		else
	...
	-- MyMod
	numNationalWonderButtons = #listNational;
	...
	-- MyMod
	table.sort(listNational, GenericSort);
	...
	-- MyMod
	for _ , wonder in ipairs(listNational) do
		AddProductionButton(wonder.ID, wonder.Description, wonder.OrderType, wonder.Cost, 10, wonder.Disabled, wonder.YieldType);
	end
	...
	-- MyMod
	if nationalHeadingOpen then
		local localizedLabel = "[ICON_MINUS] "..Locale.ConvertTextKey( "TXT_KEY_WONDER_SECTION_2" );
		Controls.NationalWondersButtonLabel:SetText(localizedLabel);
		Controls.NationalWonderButtonStack:SetHide( false );
	else
		local localizedLabel = "[ICON_PLUS] "..Locale.ConvertTextKey( "TXT_KEY_WONDER_SECTION_2" );
		Controls.NationalWondersButtonLabel:SetText(localizedLabel);
...
function AddProductionButton( id, description, orderType, turnsLeft, column, isDisabled, ePurchaseYield )	
	...
	-- MyMod
	elseif column == 2 or column == 3 or column == 10 then -- we are a building, wonder, or project
		if column == 2 then
		...
		elseif column == 10 then
			controlTable = g_NationalInstanceManager:GetInstance();
		end
